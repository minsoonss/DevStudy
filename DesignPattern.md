...
2. Abstract Factory
   ...
   ex) 컴퓨터를 생산할 때, 램 생산, 하드디스크 생산 등 관련 객체들을 다 추상화

   ​		애플컴퓨터 생성 이라는 명령을 내리게되면 관련 구성품들이 한번에 생성
...
합하여 더 큰 구조를 만들어내는데 이용

1. 어댑터

   Wrapper 패턴. 한 겹 싸서 재사용([effectiveJava18 참고](Java/EffectiveJavaStudyeffectiveJava18.md))
...
5. 퍼사드
   ...
   ex)TV를 보면된다? -> 내부 로직이 어떻게 돌아가든 TV를 보면되는
...
클래스나 객체들이 상호작용하는 방법, 책임을 분산하는 방법에 대해 정의하는 패턴
